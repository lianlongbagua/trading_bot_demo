...
async def fetch_interval_data(instId: str, interval: str, limit: int) -> Dict:
    try:
        ...
        return await asyncio.to_thread(marketDataAPI.get_candlesticks, instId=instId, bar=interval, limit=limit)
    except Exception as e:
        logger.error(f"Error fetching data for {instId} at interval {interval}: {str(e)}")
        raise
...
async def fetch_data(instrument_info: Dict) -> Dict[str, Contract]:
    ...
    process_tasks = [convert_to_Contract(raw_result, interval, instId) 
    ...
    logger.info(f"Fetched data for {len(data_dict)} intervals in {end_time - start_time:.2f} seconds")
    return data_dict
